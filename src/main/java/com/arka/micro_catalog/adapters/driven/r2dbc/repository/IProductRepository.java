...
public interface IProductRepository extends ReactiveCrudRepository<ProductEntity, Long> {
    ...
    @Query("SELECT * FROM tb_products WHERE (:search IS NULL OR name ILIKE CONCAT('%', :search, '%') OR description ILIKE CONCAT('%', :search, '%')) ORDER BY " +
            "CASE WHEN :sortDir = 'asc' THEN name END ASC, " +
            "CASE WHEN :sortDir = 'desc' THEN name END DESC " +
            "LIMIT :size OFFSET :offset")
    Flux<ProductEntity> findAllPagedWithSearch(@Param("search") String search,
                                                @Param("sortDir") String sortDir,
                                                @Param("size") int size,
                                                @Param("offset") long offset);

    @Query("SELECT COUNT(*) FROM tb_products WHERE (:search IS NULL OR name ILIKE CONCAT('%', :search, '%') OR description ILIKE CONCAT('%', :search, '%'))")
